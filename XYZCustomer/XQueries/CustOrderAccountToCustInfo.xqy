...
declare function local:getCustomerInfo($varCustomer as element() (:: schema-element(ns1:customer) ::), 
    ...
    <ns1:migrated_customer>
        ...
        <customer_orders>
            <order>
                {
                    $varOrders/ns2:orders/ns2:instrument_code
                }
                <instrument_code>{fn:data($varOrders/ns2:orders/ns2:instrument_code)}</instrument_code>
                <order_action>{fn:data($varOrders/ns2:orders/ns2:order_action)}</order_action>
                <price>{fn:data($varOrders/ns2:orders/ns2:price)}</price>
                <currency_code>{fn:data($varOrders/ns2:orders/ns2:price_currency_code)}</currency_code>
                {
                    if ($varOrders/ns2:orders/ns2:quantity)
                    then <quantity>{fn:data($varOrders/ns2:orders/ns2:quantity)}</quantity>
                    else ()
                }
                <date_created>{fn:data($varOrders/ns2:orders/ns2:date_created)}</date_created>
                {
                    if ($varOrders/ns2:orders/ns2:brokerage_amount)
                    then <brokerage_amount>{fn:data($varOrders/ns2:orders/ns2:brokerage_amount)}</brokerage_amount>
                    else ()
                }
                {
                    if ($varOrders/ns2:orders/ns2:brokerage_code)
                    then <brokerage_code>{fn:data($varOrders/ns2:orders/ns2:brokerage_code)}</brokerage_code>
                    else ()
                }
                {
                    if ($varOrders/ns2:orders/ns2:brokerage_percent)
                    then <brokerage_perc>{fn:data($varOrders/ns2:orders/ns2:brokerage_percent)}</brokerage_perc>
                    else ()
                }
            </order>
        </customer_orders>
